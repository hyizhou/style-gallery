...
const ui = {
  zh: {
    ...
    heroLead:
      '每种风格、布局与反馈词条都配有一整套可交互的组件标本：看图认效果，用口语别名搜出术语——毛玻璃、黑客屏、吐司提示都知道指什么。认出想要的那个，就把页面里的提示词复制给你的 AI。',
    metaStyles: (n: number) => `${n} 种设计风格`,
    metaLayouts: (n: number, m: number) => `${n} / ${m} 个布局模式`,
    metaFeedback: (n: number) => `${n} 个反馈模式`,
  ...
  en: {
    ...
    gridFeedback: 'Feedback navigation',
    brand: 'Style Gallery',
    ...
    heroLead:
      'Every style, layout and feedback entry ships with a full set of interactive component specimens: recognize the effect by eye, search the term by nickname — frosted glass, hacker screen or toast all resolve. Once you spot the one you want, copy its prompt to your AI.',
    metaStyles: (n: number) => `${n} design styles`,
    metaLayouts: (n: number, m: number) => `${n} / ${m} layout patterns`,
    metaFeedback: (n: number) => `${n} feedback pattern${n === 1 ? '' : 's'}`,
