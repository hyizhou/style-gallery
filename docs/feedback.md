...
> 「弹窗类型」是与「视觉风格 / 布局模式」并列的第三个标本大类在应用中的接入设计。当前设计状态描述，不含变更历史。

## 定位

- 与「视觉风格」「布局模式」同级的顶层大类：风格回答「长什么样」，布局回答「内容在页面里怎么摆」，弹窗类型回答「状态怎么告诉用户、任务在哪个容器里发生」。
- 命名约定：大类显示名为「弹窗类型」（英文标签 Popup Types）；路由前缀与代码标识沿用 `feedback`（`/feedback/:id`、`FeedbackPattern`），不再随显示名变动。
...
弹窗类型词条按打断强度与形态分五组（`FeedbackGroup`）：
...
- 路由命名空间：`/feedback/:id`；主页类别切换为「视觉风格 / 布局模式 / 弹窗类型」三标签。
