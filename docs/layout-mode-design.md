...
- 主页新增类别切换项（风格标本 / 布局模式），切换的是主页卡片网格的数据源。
- 布局模式详情路由：`/layouts/:id`；`#` 后路由由 HashRouter 承载（GitHub Pages 静态托管兼容）。
...
## 详情页结构（src/pages/LayoutPattern.tsx）
...
- 每个模式一个独立演示组件（`src/components/layouts/<Id>Demo.tsx`），LayoutPattern 按 id 分发——布局是"结构本身"，无法像视觉风格那样共享一套标记换皮。
